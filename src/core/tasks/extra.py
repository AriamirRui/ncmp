...
from src.core.exceptions import TaskCancelledError, sleep_interruptible
...
class ExtraTask:
    def __init__(self, session, logger, config, cancel_event=None):
        ...
        self.signer = Signer(session, "", logger, config,
                             cancel_event=cancel_event)  # task_id 为空字符串，因为上报听歌不需要
    ...
    def process_extra_tasks(self, task_id: str, progress=None) -> None:
        """处理额外的评分任务

        Args:
            task_id: 每日任务ID（上报听歌记录不需要，但要传给评分接口）
            progress: 可选回调 progress(stage, payload)，用于 UI 显示进度
        """
        try:
            extra_tasks, completed_count = self._get_extra_tasks()

            if progress:
                progress("extra_meta", {"completed": completed_count, "max": 15})
                ...
                try:
                    ...
                    if progress:
                        progress("extra_progress", {
                            "done": completed_count + success_count,
                            "max": 15,
                        })
                    ...
                    if success_count < remaining_tasks:  # 最后一个任务不需要等待
                        ...
                        sleep_interruptible(delay, self.cancel_event)
